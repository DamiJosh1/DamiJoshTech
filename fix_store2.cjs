...
const startTag = '<ShoppingBag className="w-5 h-5" />';
const endTag = '<main className="flex-1 flex flex-col w-full min-h-screen">';

const startIndex = content.indexOf(startTag);
const endIndex = content.indexOf(endTag);

if (startIndex !== -1 && endIndex !== -1) {
  // Find where {user ? ( starts after the ShoppingBag
  const innerStart = content.indexOf('{user ? (', startIndex);
  
  const newContent = content.substring(0, innerStart) + `{user ? (
...
` + content.substring(endIndex);
  ...
  console.log("Reconstructed Store.tsx!");
} else {
